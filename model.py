...
class Seq2Seq:
    ...
    def __init__(self, vocab_size, n_hidden=512, n_layers=3):
        self.learning_late = 0.00001

        self.vocab_size = 1264
    ...
    def cell(self, n_hidden, output_keep_prob):
        rnn_cell = tf.contrib.rnn.rnn_cell.BasicRNNCell(self.n_hidden)
        rnn_cell = tf.contrib.rnn.rnn_cell.DropoutWrapper(rnn_cell, output_keep_prob=output_keep_prob)
        return rnn_cell

    def build_cells(self, output_keep_prob=0.5):
        enc_cell = tf.contrib.rnn.rnn_cell.MultiRNNCell([self.cell(self.n_hidden, output_keep_prob)
                                                for _ in range(self.n_layers)])
        dec_cell = tf.contrib.rnn.rnn_cell.MultiRNNCell([self.cell(self.n_hidden, output_keep_prob)
                                                for _ in range(self.n_layers)])
